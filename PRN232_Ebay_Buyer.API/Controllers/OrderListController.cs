...
public class OrderListController : ControllerBase
{
    ...
    public OrderListController(CloneEbayDbContext db, ILogger<OrderListController> logger)
    {
        _db = db;
        _logger = logger;
    }
        ...
        [FromQuery] int pageSize = 5)
    {
        ...
        {
            // Kiểm tra xem có cancel request pending không
            var cancelRequest = o.ReturnRequests
                .FirstOrDefault(r => r.Reason != null && r.Reason.StartsWith("[Cancel]"));
            ...
            return new OrderSummaryResponse(
                ...
                o.TotalPrice,
                o.OrderItems.Count,
                cancelRequest?.Status  // null nếu chưa có, "Pending"/"Approved" nếu có
            );
        }).ToList();
    ...
    private static OrderDetailResponse MapToDetail(OrderTable order)
    {
        ...
        if (rr is not null)
        {
            ...
            // Tách ảnh đính kèm ra khỏi reason (nếu có)
            List<string>? imageUrls = null;
            var imgMarker = "\n[IMAGE_DATA]";
            var imgIdx = reason.IndexOf(imgMarker, StringComparison.Ordinal);
            if (imgIdx >= 0)
            {
                var jsonPart = reason[(imgIdx + imgMarker.Length)..];
                reason = reason[..imgIdx].Trim();
                try
                {
                    imageUrls = System.Text.Json.JsonSerializer.Deserialize<List<string>>(jsonPart);
                }
                catch { /* bỏ qua nếu parse lỗi */ }
            }

            returnInfo = new ReturnRequestInfo(rr.Id, type, reason, rr.Status, rr.CreatedAt, imageUrls);
        }
